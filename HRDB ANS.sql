...
select first_name, department_id, salary from employees where commission_pct is not null
order by salary desc;

-- 11. Display the last name and job title of all employees who do not have a manager 
select last_name, job_title, manager_id  from employees join jobs on
employees.job_id = jobs.job_id
...
select last_name, job_title, salary from employees join  jobs on
...
select max_salary, min_salary, commission_pct, avg(salary) from employees join jobs 
...
select sum(salary), manager_id from employees where manager_id is not null
group by manager_id;
...
select last_name from employees limit 1,5;
...
select last_name,
case 
	when salary/2 > 10000  then (salary * 10/100)
    else salary * 11.5/100
    end as bonus
    from employees;
...
select space(street_address) from locations;
...
select First_name, lower(concat(first_name,last_name,'@systechusa.com')) as 'E-mail address' from employees;
...
select first_name, min_salary from employees join jobs
on employees.job_id = jobs.job_id
order by min_salary;
...
select * from employees where first_name = 'FILLmore';
...
jobs.job_id = employees.job_id
join job_history on 
job_history.employee_id = jobs.job_id;
right join departments on 
departments.department_id = job_history.department_id;
where job_title = department_name;
...
select e.employe_id, salary, department_name ,
case
	when department_id between 10 and 30 then '5%'
    when department_id = 20  then '10%'
    when department_id between 40 and 50 then '15%'
...
update employees set commission_pct = 0 where commission_pct = null ;
